...
- **`CargoScene.tsx`** — R3F `<Canvas>` in `frameloop="demand"` mode. Splits `ship.cargoBays` into `individualBays` and `compoundBays`. Renders `CargoBayMesh` for individual bays and `CompoundBayMesh` for compound bays. Contains `centerCell()` which offsets the hovered cell so the drag preview is centered on the cursor (subtracts `Math.floor(dim/2)` on the two free axes for each anchor type, clamped to bay bounds). `resolveBayForStack()` returns `anchorFace: undefined` for compound bays so callers always get the field without a second lookup. **Drag escape handling**: a `window.pointerup` listener (active only while `draggedCrateId` is set) cancels the drag if the pointer is released outside the canvas — prevents `OrbitControls` from staying locked. A `dragHandledRef` boolean guard prevents double-firing when the drop lands on a bay mesh (R3F synthetic event + DOM window event fire in the same synchronous tick before React re-renders); bay meshes receive `wrappedOnEndDrag` which sets the flag before calling `onEndDrag`. Passes `isAssigningRef` to every `CrateMesh` so clicks on placed crates propagate to the bay mesh behind them when in assignment mode.
- **`CargoBayMesh.tsx`** — renders a single bay wireframe, grid, and an engraved flat label on the anchor face. The anchor face is rendered as a fully opaque filled panel (`#0c2840`). Two-mesh strategy for lateral faces (`left`, `right`, `front`, `rear`): a `BackSide` visual mesh (no event handlers → excluded from R3F raycast) makes the face opaque from both sides without breaking pointer event routing; the interaction plane keeps `FrontSide` so only the exterior direction receives clicks/hover. `floor`/`ceiling` use `DoubleSide` on the single interaction plane (visible from below, no directionality issue since floor planes don't face each other). In assign+hover mode the face blends to orange (`#e07828`, 35 % opacity) on both meshes. **Label ordering constraint**: `labelRot` must be declared *after* the `plane` constant — it references `plane.rot` for lateral anchors, and declaring it before `plane` triggers a TDZ ReferenceError that crashes all lateral-anchor bays. **Label visibility**: the label mesh uses `side={THREE.FrontSide}` so it is only visible from the exterior of the anchor face (back-face culled when the camera is behind the face).
- **`CompoundBayMesh.tsx`** — renders the merged wireframe + fill highlight (single geometry each, no internal seams), per-section grids (only for sections at z=0), and a single shared engraved flat label on the floor. Floor sections (`localOffset.z === 0`) use an opaque `DoubleSide` interaction plane (`#0c2840`); elevated sections keep `opacity={0}` to avoid a floating panel inside the compound volume. The label uses `side={THREE.FrontSide}` (visible from above only).
- **`CrateMesh.tsx`** — renders a placed crate; memoised, uses stable callback refs to avoid re-renders. Accepts `isAssigningRef?: MutableRefObject<boolean>`: when the ref is `true`, `onClick` and `onPointerDown` skip `stopPropagation()` so R3F's raycast continues to the bay mesh behind the crate — this lets users click on crates to place new ones in the same bay during assignment mode.
...
- **`useLabelTexture.ts`** — shared hook that generates a canvas-based `THREE.CanvasTexture` for bay labels (used by both `CargoBayMesh` and `CompoundBayMesh`). Style: bold 72 px Arial, no background or border, shadow offset (+2,+2) at 45 % opacity then main text at 82 % opacity — "engraved" appearance. Returns `{ texture, ratio }` where `ratio = canvasW / canvasH`, used by callers to fit the label within the anchor face without overflow.
...
- **`ContractForm.tsx`** — collapsible panel (starts closed); expands automatically when editing a contract. Add/edit a contract: name, color auto-assigned, deliveries with destination/commodity/SCU via `SearchableSelect`, max container size, required pickup location. Accepts `forceOpen?: number` and `forceClose?: number` counter props — the tutorial increments these to programmatically open/close the panel; using counters (not booleans) ensures the `useEffect` fires even on repeated navigation to the same step.
